      ...
      <div className="bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-all group h-full border border-gray-100 flex flex-col">
        ...
        <div className="p-5 flex-1 flex flex-col">
          <div className="flex-1">
            ...
            <h3 className="font-semibold text-gray-900 group-hover:text-blue-700 transition-colors text-lg mb-2">{property.title}</h3>
          ...
          <div className="flex flex-wrap gap-2">
            {property.amenities.slice(0, 3).map((amenity, index) => (
              <span key={index} className="bg-blue-50 text-blue-700 text-xs px-3 py-1 rounded-full border border-blue-200">
                {amenity}
              </span>
            ))}
            {property.amenities.length > 3 && (
              <span className="bg-gray-50 text-gray-700 text-xs px-3 py-1 rounded-full border border-gray-200">
        ...
        <div className="px-5 py-3 bg-white border-t border-gray-100 mt-auto">
          <button className="w-full text-sm font-medium text-blue-700 hover:text-blue-800 flex items-center justify-center group-hover:underline">
            View Details
