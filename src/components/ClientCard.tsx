...
export default function ClientCard({ client, isMem0Data = false }: ClientCardProps) {
  ...
  return (
    <Link href={`/clients/${client.id}`}>
      <div className={`bg-white rounded-xl shadow-sm hover:shadow-md transition-all p-5 border ${isMem0Data ? 'border-blue-200' : 'border-gray-100'} h-full group relative`}>        
        <div className="flex items-start justify-between">
          <div className="flex">
            <div className={`h-14 w-14 rounded-full overflow-hidden bg-gray-100 mr-4 border-2 ${isMem0Data ? 'border-blue-300' : 'border-white'} shadow-sm`}>
              <Image
        ...
        <div className="mt-4 space-y-3">
          <div className="flex items-center text-sm text-gray-600">
            <Phone size={15} className="mr-2 text-gray-400" />
        ...
        {client.memories.length > 0 && (
          <div className="mt-4 pt-4 border-t border-gray-100">
            ...
            <div className={`${isMem0Data ? 'bg-blue-50 border-blue-100' : 'bg-gray-50 border-gray-100'} p-3 rounded-lg text-sm text-gray-700 border`}>
